...
annot = pd.read_csv("localization-ground-truth-boxes.csv")
...
for i, row in annot.iterrows():
    if row["image"].startswith("cat1"):
        path = os.path.join("trainingimages", "Categorie1", row["image"])
    else:
        path = os.path.join("trainingimages", "Categorie2", row["image"])
    ...
    cv2.imwrite(os.path.join("testimages", row["image"]), plate)

    ious.append(iou)

ious = [iou for iou in ious if iou > 0.6]

print(f"accuracy: {len(ious) / len(annot)}, computed on {len(annot)} examples, number of 0 plates detected {nans}")
